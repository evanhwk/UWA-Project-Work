...
void display( void )
{
	...
	// +-------------------------------------------------------------------------------------------------------------------------------------+
    	if (sceneObjs[i].meshId >= 56 && sceneObjs[i].isPaused != 1) {	

		// Avoiding math errors
            	if (sceneObjs[i].mSpeed <= 0.0) break;
		if (sceneObjs[i].mDistance <= 0.0) break;
		
		// set currentTime [in ms]
	    	GLfloat currentTime = glutGet(GLUT_ELAPSED_TIME);

		float runTime = (float) glutGet(GLUT_ELAPSED_TIME) / 1000.0;			// Time in seconds		
		float cycle = 2 * sceneObjs[i].mDistance / sceneObjs[i].mSpeed;			// Time for one cycle		
		...
		// cos(theta)+1			2   1.5	  1    0.5   0    0.5    1     1.5	2
		// Frame #			0    11   0    11    0     11    0     11       0
		//				 forwards   backwards    backwards     forward
		// sin(theta)			0    0.5  1    0.5   0    -0.5   -1    -0.5     0
		// 			        (min)    (max)    (min)         (-max)	      (min)
					        
		sceneObjs[i].poseTime = fmod( (cos(phase * M_PI) +1 )*  sceneObjs[i].animLen, sceneObjs[i].animLen);	
		vec4 translation = 0.0;
		translation.z = sceneObjs[i].mDistance * sin(phase * M_PI);			// SIN
		mat4 rotation =  RotateX(sceneObjs[i].angles[0]) * RotateY(sceneObjs[i].angles[1]) * RotateZ(sceneObjs[i].angles[2]);
		if (sceneObjs[i].animType == 1)
			rotation *= RotateX( fmod((float)(currentTime*0.05), 360.0) ) * RotateY( fmod((float)(currentTime*0.2), 360.0) );

		displacement = 0.025 * rotation * translation;			
		sceneObjs[i].loc += displacement;						// (add displacement to loc)
	}
	// +-------------------------------------------------------------------------------------------------------------------------------------+
...
static void adjustMSpeedDistance(vec2 by)
{
    ...
    if (mSpeedTemp > 0.01 && mSpeedTemp < 2.0)
    	sceneObjs[toolObj].mSpeed = mSpeedTemp;
    if (mDistanceTemp > 0.5 && mDistanceTemp < 4.0)
...
static void mainmenu(int id)
{
    ...
    if (id >= 60 && currObject>=0) {
        toolObj=currObject;
        setToolCallbacks(adjustMSpeedDistance, mat2(0.1, 0, 0, 0.1),
                         adjustMSpeedDistance, mat2(0.5, 0, 0, 0.5));
    }
