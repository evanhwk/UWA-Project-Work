...
void init( void )
{
    ...
    sceneObjs[2].loc = vec4(-3.0, 5.0, -1.5, 0.0);		// w = 0.0
    sceneObjs[2].scale = 0.1;
    sceneObjs[2].texId = 0;	// Plain texture
    sceneObjs[2].brightness = 0.35;
    //  +----------------------------------------------------------------------+
...
static void adjustSpecularShine(vec2 by)
{
    ...
    if (specularTemp > 0.0)
    	sceneObjs[toolObj].specular = specularTemp;
    if (shineTemp > 0.0)
    	sceneObjs[toolObj].shine = shineTemp;
}
// +---------------------------------------------------+
...
{
    // MODIFIED <1K> - Keyboard Shortcuts
...
void timer(int unused)
{
    ...
    // +-------------------------------------------------------------------------------------+
    sprintf(title, "%s %s: %d Frames Per Second @ %d x %d | Selected Object ID: <%i/%i>",
                    lab, programName, numDisplayCalls, windowWidth, windowHeight, toolObj, nObjects-1);
    // +-------------------------------------------------------------------------------------+
