...
{
    setToolCallbacks(adjustCamrotsideViewdist, mat2(100,0,0,-10),
                     adjustcamSideUp, mat2(100, 0, 0,-90) );
}
...
void init( void )
{
    ...
    sceneObjs[2].loc = vec4(3.0, 1.5, 1.5, 0.0);	// w = 0.0
...
void drawMesh(SceneObject sceneObj)
{
    ...
    /* mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale); */
    	...
    	// MODIFIED
	// Implementation of Part 1B - Adding Rotation
    	// +-----------------------------------------------------------------------------------------+
...
void display( void )
{
	...
	// +--------------------------------------------------------------------------------------------------+
    	...
    	vec4 lightPosition2 = view * lightObj2.loc;	// Directional light (w=0)
	...
	glUniform3fv( glGetUniformLocation(shaderProgram, "LightBrightness1"), 1, lightObj1.brightness);
	glUniform3fv( glGetUniformLocation(shaderProgram, "LightBrightness2"), 1, lightObj2.brightness);
	// +--------------------------------------------------------------------------------------------------+
    CheckError();
...
static void mainmenu(int id)
{
    ...
    setToolCallbacks(adjustAngleYX, mat2(100, 0, 0, 400),
		adjustAngleZTexscale, mat2(-100, 0, 0, 15) );
    }
    // +------------------------------------------------------+
    if (id == 99) exit(0);
}
...
void keyboard( unsigned char key, int x, int y )
{
    ...
    case 161:			// Q - Camera Rotate
        doRotate(); break;
    case 172:			// Z - Position/Scale Object
	toolObj=currObject;
    	setToolCallbacks(adjustLocXZ, camRotZ(),
    		adjustScaleY, mat2(0.05, 0, 0, 10) );  
	break;
    case 170:			// X - Rotation/Texture Scale Object
    	setToolCallbacks(adjustAngleYX, mat2(100, 0, 0, -400),
		adjustAngleZTexscale, mat2(100, 0, 0, 15));
	break;
    case 141:			// A - Move Light 1
	toolObj = 1;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2( 1.0, 0.0, 0.0, 10.0));
	break;
    case 163: 			// S - Move Light 2
        toolObj = 2;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2( 1.0, 0.0, 0.0, 10.0));
    	break;
    case 144:			// D - RGB/All Light 1
	toolObj = 1;
        setToolCallbacks(adjustRedGreen, mat2(1.0, 0, 0, 1.0),
                         adjustBlueBrightness, mat2(1.0, 0, 0, 1.0));
	break;
    case 146:			// F - RGB/All light 2
	toolObj = 2;
        	setToolCallbacks(adjustRedGreen, mat2(1.0, 0, 0, 1.0),
                         	adjustBlueBrightness, mat2(1.0, 0, 0, 1.0));
	break;
    ...
    // +------------------------------------------------------------------------+    
    case 040:			// Enter - Pause/Start Animation
        if (sceneObjs[currObject].isPaused == 0) {
            sceneObjs[currObject].pauseTime = sceneObjs[currObject].animTime;
            sceneObjs[currObject].isPaused = 1;
            break;
