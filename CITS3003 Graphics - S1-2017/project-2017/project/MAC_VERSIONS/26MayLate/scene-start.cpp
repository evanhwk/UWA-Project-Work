...
#include "gnatidread2.h"	// MODIFIED <2> - Animation Header File [from addingAnimation.txt]
...
int numDisplayCalls = 0; // Used to calculate the number of fr// MODIFIED
ames per second
...
const aiScene* scenes[numMeshes]; // MODIFIED <2> - Animation [from addingAnimation.txt]
...
typedef struct {
    ...
    // MODIFIED <2> - Animation variables
    // +-----------------------------------------------------------------------------------------+
...
GLfloat currentTime = 0.0; // MODIFIED <2> - Animation variables
...
void loadMeshIfNotAlreadyLoaded(int meshNumber)
{
    ...
    // MODIFIED <2> - Animation [from addingAnimation.txt]
    // +----------------------------------------+
...
{
    if (nObjects != maxObjects) {				// Potential bug fix
...
void init( void )
{
    ...
    // MODIFIED <2> Animation - boneTransforms in vstart.glsl
    ...
    sceneObjs[1].brightness = 1.20; // The light's brightness is 5 times this (below).

    // MODIFIED <1I> - Adding second *directional* light
    // +-----------------------------------------------------------------------+
    // Object 2 is tthe second light.
    addObject(55); // Sphere for second light
    sceneObjs[2].loc = vec4(-3.0, 1.5, -1.5, 1.0);
...
{
    numDisplayCalls++;
    currentTime = glutGet(GLUT_ELAPSED_TIME); // MODIFIED <2> - Animation Variables
...
static void adjustSpecularShine(vec2 by)
{
    ...
    if (sceneObjs[toolObj].shine > 0.0)
...
{
	if(currObject < nObjects) currObject++;
}
...
static void makeMenu()
{
    ...
    glutAddMenuEntry("Position/Scale Object", 41);
    glutAddMenuEntry("Rotation/Texture Scale Object", 55);
...
{
    // MODIFIED <1J> - Keyboard Shortcuts
    // Additional Keyboard shortcuts for commonly used menu items
    // +------------------------------------------------------------------------+
    switch ( key ) {
    case 033:			// ESCAPE
        exit( EXIT_SUCCESS ); break;
    case 121:			// Q - Camera Rotate
        doRotate(); break;
    case 132:			// Z - Position/Scale Object
	toolObj=currObject;
    	setToolCallbacks(adjustLocXZ, camRotZ(),
    		adjustScaleY, mat2(0.05, 0, 0, 10) );  
	break;
    case 130:			// X - Rotation/Texture Scale Object
    	setToolCallbacks(adjustAngleYX, mat2(400, 0, 0, -400),
		adjustAngleZTexscale, mat2(400, 0, 0, 15));
	break;
    case 101:			// A - Move Light 1
	toolObj = 1;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2( 1.0, 0.0, 0.0, 10.0));
	break;
    case 123: 			// S - Move Light 2
        toolObj = 2;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2( 1.0, 0.0, 0.0, 10.0));
    	break;
    case 104:			// D - RGB/All Light 1
	toolObj = 1;
        setToolCallbacks(adjustRedGreen, mat2(1.0, 0, 0, 1.0),
                         adjustBlueBrightness, mat2(1.0, 0, 0, 1.0));
	break;
    case 106:			// F - RGB/All light 2
	toolObj = 2;
        	setToolCallbacks(adjustRedGreen, mat2(1.0, 0, 0, 1.0),
                         	adjustBlueBrightness, mat2(1.0, 0, 0, 1.0));
	break;
    // +------------------------------------------------------------------------+ 
    // MODIFIED <2> - Animation Pausing
    // +------------------------------------------------------------------------+    
    case 015:			// Enter - Pause/Start Animation
...
// MODIFIED <1J> - Keyboard Shortcuts
// +----------------------------------------------------------------------------+
// Function which handles modifier arrow key input
void ArrowKeys(int key, int x, int y)
{
    switch(key)
    {
	case GLUT_KEY_LEFT:
		removeObject(); break;	// Remove current object
    	case GLUT_KEY_UP:
    		nextObject(); break;	// Select next object as current object (within bounds)
    	case GLUT_KEY_DOWN:
    		previousObject(); break;		// Select previous object as current object (within bounds)
	case GLUT_KEY_RIGHT:
		addObject(rand() % numMeshes); break; // Adds random object
    }
}
// +----------------------------------------------------------------------------+
...
int main( int argc, char* argv[] )
{
    ...
    glutSpecialFunc(ArrowKeys); // MODIFIED <1J> Additional Functionality - Keyboard Shortcuts
    glutIdleFunc( idle );
