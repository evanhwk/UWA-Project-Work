...
/*	!!!!!!!!!!!!!! FOR PART TWO
	// MODIFIED
	GLuint vBoneIDs, vBoneWeights, uBoneTransforms; // Animation ID
	*/
...
const int maxObjects = 1024; // Scenes with more than 1024 objects seem unlikely
	...
	static float piradians = M_PI / 180;
	// +--------------------------------+
	//
...
void init( void )
{
	...
	*/
    ...
    sceneObjs[1].brightness = 0.2; // The light's brightness is 5 times this (below).
    	...
    	// MODIFIED
	// Implementation of I - Adding second directional light
    	// +-----------------------------------------------------------------------------------------+
...
void drawMesh(SceneObject sceneObj)
{
    ...
    /* mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale); */
    	...
    	// MODIFIED
	// Implementation of Part B - Adding Rotation
    	// +-----------------------------------------------------------------------------------------+
    	mat4 model = (RotateX(sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(sceneObj.angles[2])) * Scale(sceneObj.scale) * Translate(sceneObj.loc);
	// +-----------------------------------------------------------------------------------------+
...
void display( void )
{
	...
	// +--------------------------------------------------------------------------------------------------+
    	view = (RotateX(camRotUpAndOverDeg) * RotateY(-camRotSidewaysDeg)) * Translate( 0.0, 0.0, -viewDist);
	// +--------------------------------------------------------------------------------------------------+
    ...
    glUniform4fv( glGetUniformLocation(shaderProgram, "LightPosition"),
    	...
    	// MODIFIED
	// Implementation of I - Adding second light
	// +--------------------------------------------------------------------------------------------------+
    	SceneObject lightObj1 = sceneObjs[1]; 
	...
	drawMesh(sceneObjs[i]); // !!!!!!!!!!!!!! To be disabled
		...
		if (sceneObjs[i].isPaused != 1) {
			...
			mat4 rotation;
				...
				translate = vec4(0.0, 0.0, (-0.5 * sceneObjs[i].moveDistance * sin(M_PI * 2 * numCycles)), 0.0);
				rotate =  RotateX(sceneObjs[i].angles[0]) * RotateY(sceneObjs[i].angles[1]) * RotateZ(sceneObjs[i].angles[2]);
...
static void adjustBrightnessY(vec2 by)
{
	...
	// MODIFIED
	//  Implementation of C - Adjust Diffuse/Specular/Brightness/Ambient
...
static void makeMenu()
{
	...
	// Additional Functionality - Removing and Duplicating Objects
	// Attaching new menu items to lightmenu
    	// +-------------------------------------------------------------------------------------+
...
void reshape( int width, int height )
{
    ...
    */
    	// MODIFIED
	// Implementation of D - Adjusting near clipping plane distance
	// Implementation of E - Adjusting reshape callback function to continue to show same objects when window size is modified
    	// +-------------------------------------------------------------------------------------+
    	GLfloat nearDist = 0.03;
    	if (width < height) {
