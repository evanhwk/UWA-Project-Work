...
{
	/* init */
	if (!D_V_Ptr->init && !fl_UNLINK) {	/* if we are NOT unlinking and dir vect has not been init */
		DIRECTORY_VECTOR = init_dir_vector();		// init DIRECTORY_VECTOR 
	}
	dirPath = strcat(dirPath, "/");		// APPEND / to the end of the given directory path string
	int ERROR_SAV = 0;			// For memory of non-fatal errors
	...
	if (dir == NULL) { 			// fatal error checking
		perror(dirPath);		
		exit(EXIT_FAILURE);		// if error EXIT
	}
	else {
		...
		{		
			...
			if (stat(fullPath, stat_Ptr) == -1) { 		/* CALL stat + check errors*/
				...
				if ( strcmp(d_Ptr->d_name,".") && strcmp(d_Ptr->d_name,"..") ) { 						// (Ignoring . and .. directories)
					if ((fl_SHOW_HIDDEN || d_Ptr->d_name[0] != '.')) {							// Determine if Hidden (.) Folders are ignored
						...
						if (depthLimit < 0 || depth < depthLimit) {							/* CHECK FOR RECURSION */
							ERROR_SAV = parse_dir(statExpress, fullPath, depth+1, depthLimit); 				// Recursive call w/ depth+1 + catch any errors
						}									  										
						if (fl_UNLINK && evaluate_stat_expression(statExpress, d_Ptr->d_name, stat_Ptr)) {		/* If unlinking - STAT EXPRESSION CHECK */	
							if (rmdir(fullPath)!=0) {									// Unlink the directory **after** the recursion check (we are at the lowest dir) 
								perror(fullPath);
								ERROR_SAV = errno;									// catch errno for rmdir
							}
						}
						free(fullPath);
					}
				...
				if (fl_SHOW_HIDDEN == true || d_Ptr->d_name[0] != '.' )	{							// Ignore Hidden Items (Starting with .) unless specified otherwise
					if (evaluate_stat_expression(statExpress, d_Ptr->d_name, stat_Ptr))	{				/* STAT EXPRESSION CHECK */
						if (!fl_UNLINK) {											/* IF NOT UNLINKING */
							append_dir_vector(D_V_Ptr, fullPath, stat_Buff, depth);							//  Append directory data to the DIR_V
						}
						else {													/* IF UNLINKING */			
							if (unlink(fullPath)!=0) {										// Unlink
								perror(fullPath);
								ERROR_SAV = errno;										// catch errno for unlink
							}
						}
						free(fullPath);											// frees memory allocated to fullpath
					}
				}
			}
		}
...
*/
static void printLongList(int i)
{
	...
	strftime(m_timedate, TIMEDATE_L, "%a %b %e %T %Y", localtime(&(D_V_Ptr->file[i].fStat.st_mtime)));		// Get time/date stamp
														/* PRINTING */
...
/*  Sorting and Printing DIRECTORY_VECTOR (dependant on option flags) to the command line
	...
	then Prints output
	RETURNS either 0 or error code if errors have occurred
*/
void sort_print()
{															/* DO NON-SORT CASES FIRST */
	...
	{														/* SORTING THE DIRECTORY VECTOR */
		if (fl_SORT_TIME) {
			...
			sort_dir_vector(D_V_Ptr, SIZE);											// sort SIZE Mercurial 
		}
		else {
