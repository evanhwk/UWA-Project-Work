...
static int cmpTime (const void *a, const void *b) 
{
	const FILE_INFO *ia	= (const FILE_INFO *)a;
	const FILE_INFO *ib	= (const FILE_INFO *)b;
	double result = difftime(ib->fStat.st_mtime, ia->fStat.st_mtime);
	if (result < 1 && result > -1) {	 									// if modification time in seconds (ignoring decimals) is the same for both ib->fStat & ia->fStat
		return cmpName(a, b);											// sort by name
	}
	return (int) result;
}
...
static int cmpSize (const void *a, const void *b) 
{
	...
	else {															// if size in bytes is the same, 
		return cmpName(a, b);												// sort by name
	}
}

/* Parses enum sortType in order to determine how the referenced DIR_V should be sorted
 * Uses qsort library function for sorting, which requires sorting functions, as provided above.
*/
