...
#define DEFAULT_DEPTH -1				// No Depth limit by default ( see dir_to_vector() )
...
int main(int argc, char **argv)
{
	...
	notOpt = init_opt(argc, argv, flags, &depth);									// Checking Options
	parse_flags(flags);	
	...
	#endif
	dir_to_vector(initDirPath, statExpress, depth);									// CREATE directory vector
	#if (EVAL)
		printf("\n");
	#endif
