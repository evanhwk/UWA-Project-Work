...
int main(int argc, char **argv)
{
	...
	parse_flags(flags);											// bool array -> flags
	...
	parse_dir(statExpress, initDirPath, 0, depth_LIMIT);							// Parse data, create DIR_V [if necessary] OR unlink files
	free_stat_expression(statExpress);									// FREE Stat Express
	if ( sort_print() != 0 ) exit(EXIT_FAILURE);								// execute SORTING and PRINTING
	exit(EXIT_SUCCESS);
}
