...
void init_arg(int argc, char **argv, int notOpt, char **dirPath, char **statExpress)
{
	...
	for (int notOptCount = 0; notOpt < argc; notOpt++, notOptCount++) {										// Loop through non-opt arguments
		if (notOptCount == 0) {
			...
			CHECK_ALLOC(temp, "init")		// see MACROS in cfind.h
			*dirPath = temp;
			*dirPath = argv[notOpt];
		}
...
int main(int argc, char **argv)
{
	...
	parse_dir(statExpress, initDirPath, 0, depth_LIMIT);							// CREATE directory vector
	free_stat_expression(statExpress);												// FREE Stat Express
	if ( sort_print() != 0 ) exit(EXIT_FAILURE);								// EXEC SORTING and PRINTING
	exit(EXIT_SUCCESS);
}
