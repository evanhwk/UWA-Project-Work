...
int * findNextEvent(processList *, processQueue *, disk *, int, int, int, int);

// Runs simulation of OS
void runOS(processList *pList, disk *Disk_0) 
{
	...
	while (executing) 
	{
		int *nextEvent = findNextEvent(pList, readyQ_Ptr, Disk_0, currTime, currPID, admitPID, numTermProcess);
		...
		switch (*nextEvent) 
		{
			...
			case 3 :
				...
				pList->proc[currPID].processAge += EventRunTime;							// Record Program Time  (for post processing)
					...
					printf("calling newdisk\n");
					newDiskSeek(Disk_0, currPID, currIO, IORequestSector, IORequestType);	// Call disk function newDiskSeek
			...
			case 4 :
				...
				int *record = queryDisk_infoRecall(Disk_0);									// Find which process and IO the data belongs to by calling disk function queryDisk_infoRecall			
				...
				removeDiskSeek(Disk_0);														// Remove IO request from Blocked Queue
				if (pList->proc[recordPID].reqIO[recordIO].IOType == 'W') {				// If this was a write request
					newDiskSeek(Disk_0, recordPID, recordIO, IORequestSector, 'R'); 			// Create a new read (R) request
...
int * findNextEvent(processList *pList, processQueue *Q, disk *Disk_0, int currTime, int currPID, int admitPID, int numTermProcess)
{
	...
	// Event: IO Ready
	unblockCD = queryDisk_nextSector(Disk_0);
