...
void initOSEvents(char scenPath[])
{
	...
	disk Disk_0, *Disk_0_Ptr = &Disk_0;
	readScenario(scenPath, pList_Ptr);				// call function readScenario (init.c) to handle translation text file -> variables for program
	initDisk(Disk_0_Ptr);										// Initialise disk variables - calculating time per sector, setting diskhead to sector 0, etc. (disk.c)
	runOS(pList_Ptr, Disk_0_Ptr);						// Run the OS simulation (os.c)
...
void readScenario(char scenPath[], processList *pList)
{
	...
	int numLine = 0;																		// i - Number of lines processed so far
...
void processLine (char line[], int numLine, processList *pList)
{
	...
	else 
	{
		...
		if (strcmp(eventType,"admit") == 0) 											// IF event type is admit (for this line)
		{
			...
			pList->numProcess++;																// Increase global variable keeping track of process count - g_numProcess
		}
		...
		else if (strcmp(eventType,"read") == 0) 										// ELSE IF event type is read (for this line)
		{
			...
			while (pList->proc[PID].reqIO[i].IOType) { i++; }					// If this I/O request ID has already been initialised, increase the i until null is found
