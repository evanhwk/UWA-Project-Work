...
public class CITS2200Project { //implements CITS2200Project 
    ...
    {
        if (Graph.getSize() > 20) return null;
    ...
    */
    ...
    private int[][] BHK_HamiltonPath(int adj[][]) {
	int n = adj.length;
	...
	// reconstruct path
	int cur = (1 << n) - 1;
	String[] order = new int[n];
	int last = -1;
	for (int i = n - 1; i >= 0; i--) {
		int bj = -1;
		for (int j = 0; j < n; j++) {
			if ((cur & 1 << j) != 0 && (bj == -1 || dp[bj][cur] + (last == -1 ? 0 : dist[last[bj]]) > dp[j][cur] + (last == -1 ? 0 : dist[last][j]))) {
          			bj = j;
        		}
      		}
        	order[i] = bj;
    ...
    private class Gabow
    {
        ...
        ArrayList<List<Integer>> sccList;
        Stack<Integer> st1;
        Stack<Integer> st2;
        ...
        public Gabow(CITS2200Graph g)
        {
            ...
            check = new boolean[gSize];
            sccList = new ArrayList<>();
            st1 = new Stack<Integer>();
            st2 = new Stack<Integer>();
        }
        
        public ArrayList<List<Integer>> GabowInit() 
        {
        ...
        public void dfs(int v)
        {
            ...
            LinkedList<String> links = Graph.getEdge(v);
            for (String link : links) 
            {
                int w = Graph.getIndex(link);
                if (!visited[w])
                    dfs(w);
                else if (!check[w]) {
                    while (preOrder[st2.peek()] > preOrder[w]) 
                    {
                        st2.pop();     
                    }
                }     
