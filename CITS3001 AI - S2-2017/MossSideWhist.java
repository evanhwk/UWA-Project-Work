...
import java.io.*;
/**
 * A class implementing the Moss Side Whist game.
 * @author Tim French
...
public class MossSideWhist{
  ...
  Map<String, Integer> scoreboard;  //a map from names to scores
  ...
  public void playHand(){
    ...
    //report.println(leader+",\t"+left+",\t"+right);
    deal();
    ...
    for(int i = 0; i<4; i++){
        if(i>=discard.length || !hands.get(leader).remove(discard[i])) {
            ...
            //report.println("Illegal Discard");
        }
        display(leader);
    }
    String first = leader;
  ...
  //display(first, true);
    ...
    Card lead = agents.get(first).playCard();
            ...
            if (end-start > 200) System.err.println("TIME EXCEEDED ("+(end-start)+"ms) FOR AGENT "+first);
    ArrayList<Card> hand = hands.get(first);
    if(!hand.remove(lead)) {
      lead = hand.remove(rand.nextInt(hand.size()));
      //report.println("Illegal Move");
    }
    showCards(lead, first);
  //report.print(first+" -> "+lead+", ");
  //display(second, true);  
            long start2 = System.currentTimeMillis();
    Card next = agents.get(second).playCard();
            long end2 = System.currentTimeMillis();
            if (end2-start2 > 200) System.err.println("TIME EXCEEDED ("+(end2-start2)+"ms) FOR AGENT "+second);
  ...
  //report.print(second+" -> "+next+", ");
  //display(third, true); 
            long start3 = System.currentTimeMillis();
    Card last = agents.get(third).playCard();
            long end3 = System.currentTimeMillis();
            if (end3-start3 > 200) System.err.println("TIME EXCEEDED ("+(end3-start3)+"ms) FOR AGENT "+third);
  ...
  private void showScores()
  {
    ...
    */
    int first = scoreboard.get(leader);
    int second = scoreboard.get(left);
    int third = scoreboard.get(right);
    
    int highestValue = -99999;
    String high = "";
    if (first > highestValue) {
        highestValue = first;
        high = leader;
    }
    if (second > highestValue) {
        highestValue = second;
        high = left;
    }
    if (third > highestValue) {
        highestValue = third;
        high = right;
    }
    report.print(high+"^"+" ,");
    //report.println(first+",\t"+scoreboard.get(left)+",\t"+scoreboard.get(right));
  ...
  public static void main(String[] args){
    ...
    for (int i = 0; i < iterations; i++) {
        MossSideWhist game = new MossSideWhist(new RandomAgent("A/Game1-Random"), new ISMCTS_DLSESE_Agent("B/Game1-DLEDSE"), new RandomAgent("C/Game1-Random"));
        game.playGame(3, System.out);
    }
    System.out.println();
    for (int i = 0; i < iterations; i++) {
        game = new MossSideWhist(new ISMCTS_DLSESE_Agent("A/Game2-DLEDSE"), new ISMCTS_DLSESE_Agent("B/Game2-DLEDSE"), new ISMCTS_DLSESE_Agent("C/Game2-DLEDSE"));
        game.playGame(3, System.out);
    }
    System.out.println();
    for (int i = 0; i < iterations; i++) {
        game = new MossSideWhist(new ISMCTSAgent("A/Game3-ISMCTSv2"), new ISMCTS_DLSESE_Agent("B/Game3-DLEDSE"), new ISMCTS_DLSESE_Agent("C/Game3-DLEDSE"));
        game.playGame(3, System.out);
    }
    System.out.println();
    for (int i = 0; i < iterations; i++) {
        game = new MossSideWhist(new ISMCTSAgent("A/Game4-ISMCTSv2"), new ISMCTSAgent("B/Game4-ISMCTS2_v2"), new ISMCTS_DLSESE_Agent("C/Game4-DLEDSE"));
        game.playGame(3, System.out);
    }
    System.out.println();
    for (int i = 0; i < iterations; i++) {
        game = new MossSideWhist(new ISMCTS_DLSESE_Agent("A/Game5-DLEDSE"), new ISMCTSAgent("B/Game5-ISMCTS2v2"), new RandomAgent("C/Game5-Random"));
        game.playGame(3, System.out);
    }
    System.out.println();
    */
    ...
    for (int i = 0; i < iterations; i++) {
        MossSideWhist game = new MossSideWhist(new ISMCTS_DLSESE_Agent("A/Game1-DLSESE", true, false), new Agent20916873("B/Game1-ISMCTS2v2"), new ISMCTS_DLSESE_Agent("C/Game1-ISMCTS2v2", false, false));
        game.playGame(3, System.out);
    }
  }
}
