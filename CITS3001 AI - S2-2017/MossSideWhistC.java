...
import java.io.*;
/**
 * A class implementing the Moss Side Whist game with a cheating player.
 * @author Tim French
 * @author Evan Huang 20916873
 * @version 1.2
 */
public class MossSideWhistC {
   ...
   * 
   */
  public MossSideWhistC(MSWAgent p1, String n1, MSWAgent p2, String n2, MSWAgent p3, String n3){
    //assign names
    leader = n1;
   ...
   * @param p3 the class of the third agent
   * */
  public MossSideWhistC(MSWAgent p1, MSWAgent p2, MSWAgent p3){
  ...
  public void deal() {
    ...
    // FOR CHEATING AGENT
    agents.get(leader).seeOpponents((ArrayList<Card>) hands.get(left).clone(), (ArrayList<Card>) hands.get(right).clone());
    agents.get(left).seeOpponents((ArrayList<Card>)hands.get(right).clone(), (ArrayList<Card>) hands.get(leader).clone());
    agents.get(right).seeOpponents((ArrayList<Card>) hands.get(leader).clone(), (ArrayList<Card>) hands.get(left).clone()); 
  }
  ...
  public static void main(String[] args){
    ...
    for (int i = 0; i < iterations; i++) {
        //MossSideWhist game = new MossSideWhist(new ISMCTS_DLSESE_Agent("A/Game1-DLSESE", true, false), new Agent20916873("B/Game1-ISMCTS2v2"), new MCTSAgent("C"));
        MossSideWhistC game = new MossSideWhistC(new ISMCTS_DLSESE_Agent("A"), new MCTSAgent("B"), new MCTSAgent("C"));
        game.playGame(1, System.out);
        game = new MossSideWhistC(new MCTSAgent("B"),new ISMCTS_DLSESE_Agent("A"),  new MCTSAgent("C"));
        game.playGame(1, System.out);
    }
    System.out.println();
  }
