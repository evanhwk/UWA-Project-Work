 ...
 * @version 6.0
 */
public class ISMCTS_DLSESE_Agent implements MSWAgent {
    ...
    private WState currState;                                                 // WState object representing this agent's view of the game world.
    ...
    */
    public ISMCTS_DLSESE_Agent(String name) 
    {
        this.rand = new Random();
        this.name = name;
    }
    ...
    * @param name The name of this agent.
    */
    public ISMCTS_DLSESE_Agent(String name, boolean dls, boolean ese) 
    {
        this.rand = new Random();
        this.name = name;
        this.DLSflag = dls;
    ...
    */
    ...
    {
        Card play = Search(SEARCH_TIME-5);    //5 ms buffer
        return play;
    }
    ...
    */ 
    private Card Search(long time)
    {
        ...
        long end = start;
        //iterationCount = 0;
        WNode rootNode = new WhistNode(null, null);                           // Initialise the tree with a root node          
        while ((end-start) < time) {                                              // While we are within the time limit execute this.
          WNode currNode = rootNode;
          WState currRandState = currState.cloneAndRandomize();             // Choose a new determinization d.
          /* SELECT
          ...
          while (!currNode.hasMovesToTry(currRandState) && currNode.hasChildren()) { // while !(A) && !(B)
              currNode = currNode.selectChild();                                    // use IS-UCT algorithm to select next node to select [in WNode.selectChild()].
              currRandState.doMove(currNode.getLastPlay());                          // make (simulated) move
          }
          /* EXPAND / Tree Policy
